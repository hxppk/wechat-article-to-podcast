...
6. 韵律标记 (Prosody Markers for TTS):
   - 可使用的标记（ElevenLabs audio tags，方括号格式）：[laughs] [chuckle] [sighs] [breath] [emm] [gasps]
   - 规则：
...
7. 自然收尾 (Natural Closing):
   - 全篇必须有完整的双人收尾，禁止戛然而止，禁止只有一个人结束。
   - 老王：倒数第二句给一个升华/收束性的洞察，把这期的"题眼"点出来。
   - 小李：最后一句紧接着用一句轻松、意犹未尽的话呼应作结（让"麦克风"在小李这儿关掉）。
   - 禁止电台套路腔（如"感谢收听、我们下期再见、以上就是今天的内容"）；要像两个朋友聊到尾声自然收住。
   - 示例：
     * Speaker_A: 所以说白了，这事儿的本质还是...（一句洞察）。
     * Speaker_B: 嗯...[sighs]今天这波信息量有点大，我得回去缓缓。这个咱下次接着掰扯。
